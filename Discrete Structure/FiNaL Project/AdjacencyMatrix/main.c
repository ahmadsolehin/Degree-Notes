...
void main()
{
	...
	printf("Nur Nabila Binti Rosli CA13016 \n");
	printf("Nur Syuhaidah Binti Ismail CB13006 \n");
	printf("Nor Amirah Binti Zahari CA13014 \n");
    printf("Natasha Shafira Binti Atan CA13011 \n");
...
int read_graph ( int adj_mat[50][50], int n )
{
	...
	for ( i = 1 ; i <= n ; i++ )
	{
		for ( j = 1 ; j <= n ; j++ )
		{
			...
			printf("\n Vertices %d & Vertices %d are Adjacent ? (Y/N) :",i,j);
			scanf("%c", &reply);
			if ( reply == 'y' || reply == 'Y' )
				adj_mat[i][j] = 1;
			else
